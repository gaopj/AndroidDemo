...
public class DownLoadTask extends Thread{
    private volatile FileInfo  info;
    private SQLiteDatabase db;
    private DbHelper helper;//数据库帮助类
    private int finished = 0;//当前已下载完成的进度
    private DownLoaderManager.OnProgressListener listener;//进度回调监听

    public static final int UPDATE_PROGRESS = 0x123;

    public DownLoadTask(FileInfo info, DbHelper helper, DownLoaderManager.OnProgressListener listener) {
        this.info = info;
        this.helper = helper;
    ...
    public void run() {
        ...
        try {
            ...
            //设置文件写入位置
            File file = new File(DownLoaderManager.FILE_PATH, info.getFileName());
    ...
    private void getLength() {
        ...
        try {
            ...
            //创建文件保存路径
            File dir = new File(DownLoaderManager.FILE_PATH);
            if (!dir.exists()) {
                dir.mkdirs();
            }
