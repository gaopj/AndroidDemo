package com.gpj.barchartviewdemo.barchart;
...
public class BarChartView extends View {
    ...
    private static final int BAR_GROW_STEP = 10;//柱状条增长动画步长
    private static final int DELAY = 10; // 柱状条增长动画重绘延时

    private float[] mDataList;// 柱状图数据列表
    private String[] mHorizontalAxis;// 水平方向x轴坐标
    private float mMax;// 数据集合允许最大值
    private float mNowMax; // 当前集合的最大值
    private float heightRatio; // 最大像素大小与允许最大数据值的比值

    private List<Bar> mBars = new ArrayList<>();
    private float mBarWidth; // 柱状条宽度
    private int mRadius; // 柱状条宽度1/2

    private Paint mAxisPaint; // 坐标文本画笔
    private Paint mBarPaint; // 柱状条画笔
    private Rect mTextRect; // 文本矩形框
    private RectF mTemp; // 柱状条矩形
    ...
    private void init() {
        ...
        //柱状条与坐标文本之间的间隔大小，默认6dp
        mGap = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 6, getResources().getDisplayMetrics());
    }

    /**
     * 设置水平方向x轴坐标值
     * @param horizontalAxis 坐标值数组}
     */
     ...
     * 设置柱状图数据
     * @param dataList 数据数组
     * @param max 数据数组中的最大值，最大值用来计算绘制时的高度比例
     */
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (float data : mDataList) {
            mNowMax = data>mNowMax?data:mNowMax;
    ...
    private void drawBarsWidthAnimation(Canvas canvas) {
        ...
        for (int i = 0; i < mDataList.length; i++) {
            ...
            if (bar.currentTop <= bar.top) {
                ...
                if (bar.currentTop <= getPaddingTop() + (mMax-mNowMax)*heightRatio) {
